...
async function togglePushNotifications(enabled) {
  var desc = document.getElementById('pushToggleDesc');
  if (!fcmMessaging) {
    showNotif('msg', 'G��n+�', 'Not Available', 'Push notifications not supported');
    return;
  }
  ...
  if (enabled) {
    try {
      ...
      if (permission === 'granted') {
        ...
        showNotif('msg', 'G��n+�', 'Permission Denied', 'Allow notifications in browser settings');
      }
    } catch (e) {
...
function buildBubbleContent(msg, isOwn, time, showTime) {
  ...
  if (showTime !== false) {
    var seenHtml = (isOwn && msg.seen) ? ' <span style="color:var(--ios-green);font-size:11px">G��G��</span>' : '';
...
function updateMessageInDOM(msg) {
  ...
  // Preserve msg-avatar/msg-avatar-spacer G�� find or create msg-bubble-wrap
  var bubbleWrap = wrapper.querySelector('.msg-bubble-wrap');
  if (!bubbleWrap) {
    // Lost bubbleWrap due to old bug G�� recreate structure
...
// Global typing listener G�� shows toast when OTHER user types (not in current chat)
var lastTypingNotif = {};
function listenGlobalTyping() {
  ...
  db.collection('typing').onSnapshot(function(snap) {
    snap.forEach(function(doc) {
      ...
      showNotif('typing', 'G�n+�', name, 'typing...');
    });
  });
}
...
// Global recording listener G�� shows toast when OTHER user records voice
var lastRecNotif = {};
